...
# Print the metrics
print(f"The baseline accuracy (predicting majority class): {round(baseline_accuracy, 3)}")
print(f"Micro Accuracy = Micro Precision = Micro Recall = Micro F1: {round(accuracy, 3)}")
...
```{python}
disp = ConfusionMatrixDisplay(confusion_matrix=conf_matrix, display_labels=["conflict", "negative", "neutral","positive",])
...
predictions_test = pd.DataFrame(columns=['sentence_id', 'aspect_term', 'polarity_prediction'])
...
data_test_p = pd.merge(data_test, predictions_test, how="left", on = ["sentence_id", "aspect_term"]).dropna()
