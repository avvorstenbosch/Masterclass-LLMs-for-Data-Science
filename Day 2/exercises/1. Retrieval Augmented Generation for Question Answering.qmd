...
Large Language Models have a vast amount of internal knowledge (parametric knowledge).
This constitutes the general linguistic knowledge these models have, as well as the factual knowledge they have about the world.
However, this parametric knowledge is far from flawless.
These models tend to hallucinate more, the more detailed your requests become.
Furthermore, you may have questions about:
...
This means we filtered out the sections:
...
Run the code below to get your environment started, make sure you have your API-key inside the credentials.yml, or import it in any other way how you see fit.
Install the packages below that fail to load.
...
One of these functions is called `get_embeddings`, which will retrieve embeddings for you.
These are the cheapest response object from the OpenAI-api at roughly 3000 pages per dollar.
...
They cited precedents that were hallucinated by ChatGPT.
This event took place in May 2023.
ChatGPT should not be aware of these events as they happened past the cutoff date.
Let us give it a try.
Write a query asking ChatGPT what it knows about this case.
...
For our exercise, that is good news.
As we are going to prove that given the right technique, we can get ChatGPT to tells us about this anyways.
...
These embeddings have been normalised.
Because of this, the only quality of the embedding that is informative to us is the pointing direction.
This direction says a lot about the meaning of the embedded text: - Chunks with more similair meaning have more similair embeddings, - Chunks with more different meaning have more different embeddings.
...
The projection steps have been performed for you in this cell:
...
Because our vectors are already normalised, we can use take the dot-product or inner product between 2 vectors to directly calculate the angle between them, this is the cosine distance.
...
Make another plot showing the distribution for embeddings of the user prompt, together with the following 3 input prompts:
...
Instead, change the system prompt to achieve the following: - It should only answer, if the context provides an answer.
...
If our model changes a single character, it won't work.
...
For each quote found in the model answer above: - Calculate the Levensthein distance.
...
Now that we have a way of checking whether the quotes are not hallucinations, we can adjust our prompting function to do these calculations for us: - Fill out all the helper function - Then fill in the main `RAG_completion` function:
