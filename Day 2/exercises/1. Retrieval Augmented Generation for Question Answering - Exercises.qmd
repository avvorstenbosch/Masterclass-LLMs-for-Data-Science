...
data <- data %>%
  mutate(embedding = str_replace_all(embedding, YOUR_CODE_HERE, YOUR_CODE_HERE)) %>%
...
for (i in 1:length(quotes)) {
  ...
  context_tmp <- context_tmp %>%
    rowwise() %>%
    mutate(incorrectnes_quote = round(adist(x = quotes[[i]], y = text, partial = TRUE, ignore.case = TRUE) YOUR_CODE_HERE, 2)) %>% # Finish the function at the YOUR_CODE_HERE
    mutate(correct_quote = YOUR_CODE_HERE ) %>% # Finish the function at the YOUR_CODE_HERE
    group_by(page) %>%
    summarize( # Finish these aggregations at the YOUR_CODE_HERE
      incorrectnes_quote = YOUR_CODE_HERE(incorrectnes_quote),
      correct_quote = YOUR_CODE_HERE(correct_quote),
      text = text[which.YOUR_CODE_HERE(incorrectnes_quote)],
      page = page[which.YOUR_CODE_HERE(incorrectnes_quote)]
    ) %>%
    ungroup()
