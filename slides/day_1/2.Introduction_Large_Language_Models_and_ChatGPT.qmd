...
# LLM developments - proprietary {.smaller}

| Feature/Aspect           | GPT-3.5                                      | Newer Models                                               |
|--------------------------|----------------------------------------------|-----------------------------------------------------|
...
| **Hallucinations**     | High                   | A lot lower      |
: {.custom-table }

# LLM developments - Open Weights vs Proprietary{.smaller}

| Feature/Aspect           | Proprietary (ChatGPT/Gemini/Mistral)        | Open-Weights  Models                                               |
|--------------------------|----------------------------------------------|-----------------------------------------------------|
| **Problem-Solving**      | Deep Research modes and a lot of `reasoning` compute  | Limited compute due to hardware              |
| **Speed**           | Can be fast, noticable slowdown with heavy use    | Faster, but lesser quality                      |
| **Context Length**       |  128K+ tokens                 | ~8K to ~128K tokens context                             |
| **Multimodality**        | Standard     | Available, but not default   |
| **Factual Accuracy**     | Higher, especially with tool support | low, requires RAG for most usecases   |
| **Instruction following**    | High, but less controlable settings          | Highly controlable settings, but lesser base instruction following                      |
| **Hallucinations**     | High                   | more likely to produce factual responses        |
| **Privacy** | Low, external processing by third parties | High, best option for sensitive data/information |
: {.custom-table }