...
## Task: extract the movie title, genre, year, director and actors from the review. Place this information at the top of the review.
...
Now apply the prompt you've crafted to the second movie review: `./Day 1/exercises/data/movie-review-2.txt`. Does it work?
...
```{.R}
library(re)
num_tokens_from_messages <- function(messages, model="gpt-3.5-turbo-0613") {
    ...
    for (message in messages) {
        ...
        for (key in names(message)) {
            ...
            value <- gsub(" +", " ", value)
            # Calculate tokens for each word, assuming each word is 4/3 tokens
            num_tokens <- num_tokens + (length(strsplit(value, " ")[[1]]) * 4/3)
            if (key == "name") {
